...
[ "$SMTP_USE_TLS" == YES -o "$SMTP_USE_TLS" == "True" ] && _SMTP_USE_TLS="True" || _SMTP_USE_TLS="False"
...
SITE_ID = 1
ALLOWED_HOSTS = [$ALLOWED_HOSTS, '127.0.0.1', '::1']
...
MIDDLEWARE = [
    ...
    #'django.middleware.common.BrokenLinkEmailsMiddleware', # Disable if too many 404 are reported
]
...
LOGGING = {
    ...
    'loggers': {
        'eoxserver': {
            'handlers': ['eoxserver_file', 'eoxserver_email'],
...
then
    manage social_provider import --file "$EOXS_SOCIAL_PROVIDERS"
fi
